...
databases or binary files. Once you have created data objects in R it may be necessary to
convert them from one type to another (i.e. coercion), summarise them, and to subset and
transform objects often by writing R functions.
In this document we describe the main R data types, then show how to manipulate them and imported data into R

R has a wide variety of data types including scalars, vectors, matrices, data frames and lists. These may contain variables of different types, e.g. numerical, character, logical.  See the 
\href{http://cran.r-project.org/doc/manuals/R-lang.html}{R language definition} for a complete list and documentation  and the \href{http://cran.r-project.org/doc/contrib/Short-refcard.pdf}{R reference card} for a summary of
how to use them.
...
sets as used by statistical software such as SAS and contain variables (columns) by
observation (row). The columns may contain vectors, factors, and/or matrices all having the
...
R has extensive documentation and some basic ways of finding help are shown below
...
\href{http://cran.r-project.org/doc/FAQ/R-FAQ.html}{R FAQ}
\href{http://cran.r-project.org/bin/windows/base/rw-FAQ.html}{Windows R FAQ}
\href{http://www.rseek.org}{R seek}

Mailing lists for users and developers who may want to create their own packages
\href{https://stat.ethz.ch/pipermail/r-help/2006-May/thread.html#106031}{example post \& discussion}

and 
\href{http://journal.r-project.org/}{R Journal}
\href{http://www.r-bloggers.com/}{blogs}
\href{http://www.statmethods.net/index.html}{Quick-R}

Packages also have mailing lists and guides, for example the graphics package 

\href{http://ggplot2.org}{ggplot2}

R is an object oriented language and an important concept is that everything in R is an object and that every object belongs to a class. What class an object belongs to determines what operations can be performed on it and what a generic method like plot() or or summary() produces. 
...
The vector can be summaried by calculating it mean and standard deviation and plotted as a
...
go out of a vectors bounds, since this is protected
...
\begin{mylisting}\begin{center}\begin{minipage}[H]{0.95\textwidth}\begin{shaded} 
<<eval=FALSE>>=
fileSwo=paste(dirData,"\\VPASuite\\swoCN.dat",sep="")
